        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    String status = mContext.getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                if (whichFlag) {
                                    if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                        status += " " + mContext.getString(R.string.str_printer_out_of_paper);
                                    }
                                    if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                        status += " " + mContext.getString(R.string.str_printer_open_cover);
                                    }
                                    if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                        status += " " + mContext.getString(R.string.str_printer_error);
                                    }
                                } else {
                                    if ((buffer[0] == ESC_LOW_POWER)) {
                                        status = mContext.getString(R.string.str_low_power);
                                    } else if ((buffer[0] == ESC_AMID_POWER)) {
                                        status = mContext.getString(R.string.str_amid_power);
                                    } else if ((buffer[0] == ESC_HIGH_POWER)) {
                                        status = mContext.getString(R.string.str_high_power);
                                    } else if ((buffer[0] == ESC_CHARGING)) {
                                        status = mContext.getString(R.string.str_charging);
                                    }
                                }

                                System.out.println(mContext.getString(R.string.str_state) + status);
                                String mode = mContext.getString(R.string.str_printer_printmode_esc);
                            }
                        }
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (cnt == 1) {//查询打印机实时状态
                                ...
                                System.out.println(mContext.getString(R.string.str_state) + status);
                                String mode = mContext.getString(R.string.str_printer_printmode_tsc);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                                mContext.sendBroadcast(intent);
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (cnt == 1) {
                                ...
                                String mode = mContext.getString(R.string.str_printer_printmode_cpcl);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                                mContext.sendBroadcast(intent);
...
//            System.out.println("id -> " + id);
//            reader2.cancel();
//            boolean b = this.mPort2.closePort();
//            if (b) {
//                this.mPort2 = null;
//                isOpenPort = false;
//                currentPrinterCommand = null;
    ...
    public void openPort() {
        ...
        switch (deviceConnFactoryManagers[id].connMethod) {
            ...
            case WIFI:
                ...
                break;
            case SERIAL_PORT:
                mPort = new SerialPort2(serialPortPath, baudrate, 0);
                isOpenPort = mPort.openPort();
                break;
            default:
                break;
...
//        reader2 = new PrinterReader2();
        reader.start(); //读取数据线程
//        reader2.start(); //读取数据线程
        //查询打印机所使用指令
        queryPrinterCommand(); //
    }
...
//                sendDataImmediately2(data); //发送esc数据
...
//    public void sendDataImmediately2(final Vector<Byte> data) {
//        if (this.mPort2 == null) {
//            return;
//        }
//        try {
//
//            this.mPort2.writeDataImmediately(data, 0, data.size());
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
//    }
...
//    public int readDataImmediately2(byte[] buffer) throws IOException {
//        return this.mPort2.readData(buffer);
//    }
...
//    public  class PrinterReader2 extends Thread {
//        private boolean isRun = false;
//
//        private byte[] buffer = new byte[100];
//
//        public PrinterReader2() {
//            isRun = true;
//        }
//
//        @Override
...
//                while (isRun) {
//                    //读取打印机返回信息
//                    int len = readDataImmediately2(buffer);
//                    if (len > 0) {
//                        Message message = Message.obtain();
//                        message.what = READ_DATA;
//                        Bundle bundle = new Bundle();
